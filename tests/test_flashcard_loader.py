...
class TestLoadValidFlashcards:
    def test_load_valid_flashcards_array(self, tmp_path):
    ...
    def test_load_valid_flashcards_object_format(self, tmp_path):
    ...
    def test_load_strips_whitespace_from_fields(self, tmp_path):
        content = json.dumps([{"front": "  CPU  ", "back": "  Central Processing Unit  "}])
        filepath = _write(tmp_path, "deck.json", content)
    ...
    def test_load_accepts_capitalized_keys(self, tmp_path):
        content = json.dumps([{"Front": "CPU", "Back": "Central Processing Unit"}])
        filepath = _write(tmp_path, "deck.json", content)
...
class TestLoadInvalidInput:
    ...
    def test_load_missing_front_field(self, tmp_path):
    ...
    def test_load_blank_field_is_rejected(self, tmp_path):
    ...
    def test_load_empty_deck(self, tmp_path):
        filepath = _write(tmp_path, "deck.json", "[]")

        with pytest.raises(FlashcardLoadError):
            load_flashcards(filepath)

    def test_load_unsupported_shape(self, tmp_path):
        filepath = _write(tmp_path, "deck.json", json.dumps({"not_cards": []}))

        with pytest.raises(FlashcardLoadError):
            load_flashcards(filepath)

    def test_load_non_object_card(self, tmp_path):
